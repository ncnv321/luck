...
function loadGalleryContent() {
  ...
  artworksContainer.innerHTML =
    '<div class="loading-indicator" style="text-align: center; padding: 20px;">Loading gallery...</div>'
  ...
  try {
    ...
    if (typeof sanityClient !== 'undefined') {
      ...
      artworksContainer.innerHTML =
        '<div style="text-align: center; padding: 20px;">Error: Sanity client not available. Please check the console for details.</div>'
      return
    }
    ...
    client
      ...
      .then((artworks) => {
        ...
        if (!artworks || artworks.length === 0) {
          artworksContainer.innerHTML =
            '<div style="text-align: center; padding: 20px;">No artworks found. Add some in your Sanity Studio!</div>'
          return
        }
        ...
        const orderedCategories = ['2026', '2025', '2024', '2023', '2022', 'sketches']
      ...
      .catch((error) => {
        ...
        artworksContainer.innerHTML =
          '<div style="text-align: center; padding: 20px;">Error loading gallery. Please check the console for details.</div>'
      })
  } catch (error) {
    console.error('Exception in gallery script:', error)
    artworksContainer.innerHTML =
      '<div style="text-align: center; padding: 20px;">An error occurred while setting up the gallery. Please check the console for details.</div>'
  }
}
